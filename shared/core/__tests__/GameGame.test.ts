...
describe('GameGame', () => {
    ...
    describe('getQuestions', () => {
        ...
        it('should return empty list when repository data is empty', async () => {
            const emptyGame = await GameGame.createGame(gameConfig, emptyGameAdapter) 
            expect(emptyGame.getQuestions().length).toBe(0)
        })

        it('should NOT return empty list when repository data is NOT empty', async () => {
            const game = await GameGame.createGame(gameConfig, gameAdapter);
        ...
        it('two lists should have the same elements given the same GameConfig', async () => {
            ...
            const gameA = await GameGame.createGame(gameConfig, gameAdapter)
            const gameB = await GameGame.createGame(gameConfig, gameAdapter)
